...
import { TESTIMONIALS, MITRA_LOGOS } from '../../data/content'

export function ProofStrip() {
  return (
    <section style={{ paddingBlock: '4rem', background: 'linear-gradient(180deg, rgba(209,0,113,0.02) 0%, rgba(31,129,36,0.02) 100%)' }}>
      {/* Marquee logos */}
      <div className="mb-10">
        <p
      ...
      <div className="page-wrap">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
          {[
            { icon: <MapPin className="h-5 w-5" />, end: 35, suffix: '', label: 'Provinsi Indonesia', color: 'var(--magenta-bold)' },
            { icon: <GraduationCap className="h-5 w-5" />, end: 15000, suffix: '+', label: 'Guru Tersertifikasi', color: 'var(--emerald)' },
            { icon: <Users className="h-5 w-5" />, end: 500000, suffix: '+', label: 'Siswa Aktif', color: 'var(--golden)' },
            { icon: <Calendar className="h-5 w-5" />, end: 2012, suffix: '', label: 'Berdiri Sejak', color: 'var(--deep)' },
          ].map((stat) => (
            <div
              key={stat.label}
