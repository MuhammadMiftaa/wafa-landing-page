...
import { SectionHeading, Badge } from '../ui/index'
...
export function CommunityMapSection() {
  ...
  const filteredDaerah = WAFA_DAERAH.filter(d =>
    search === '' ||
  ...
  const groupedFiltered = filteredDaerah.reduce<Record<string, typeof WAFA_DAERAH>>((acc, d) => {
      ...
      <div className="page-wrap">
        <div className="text-center mb-12">
          ...
          <p className="max-w-xl mx-auto text-base leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
            Dari Sabang sampai Merauke, Wafa hadir untuk memastikan setiap anak Indonesia
            bisa belajar Al-Qur'an dengan mudah dan menyenangkan.
          </p>
        </div>
        ...
        >
                ...
                return (
                  <g key={prov}>
                    <circle
                      ...
                      fill={isActive
                        ? (isHovered ? 'var(--magenta-bold)' : 'var(--emerald)')
                        : 'rgba(157,22,124,0.15)'
                      }
                      style={{
                        transition: 'all 200ms ease',
                        cursor: isActive ? 'pointer' : 'default',
                        filter: isHovered ? 'drop-shadow(0 0 6px rgba(209,0,113,0.5))' : 'none',
                      }}
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ opacity: 0.8, fontSize: '9px' }}>
                      {WAFA_DAERAH.filter(d => d.provinsi === hoveredProvinsi).length} wilayah aktif
                    </div>
                  </div>
                </foreignObject>
              )}
          ...
          {/* Legend */}
          <div className="flex flex-wrap items-center justify-center gap-6 mt-4 pt-4" style={{ borderTop: '1px solid var(--line)' }}>
            <div className="flex items-center gap-2 text-xs" style={{ color: 'var(--text-secondary)' }}>
              <div className="h-3 w-3 rounded-full" style={{ background: 'var(--emerald)' }} />
              Provinsi dengan Wafa Daerah
            </div>
            <div className="flex items-center gap-2 text-xs" style={{ color: 'var(--text-secondary)' }}>
              <div className="h-3 w-3 rounded-full" style={{ background: 'rgba(157,22,124,0.15)' }} />
          ...
          <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
            {Object.keys(groupedFiltered).length === 0 ? (
              <div className="text-center py-12">
                <MapPin className="h-10 w-10 mx-auto mb-3" style={{ color: 'var(--line)' }} />
              ...
              Object.entries(groupedFiltered).map(([provinsi, daerahList]) => (
                ...
                >
                  <div className="flex items-center gap-3 px-5 py-3" style={{ borderBottom: '1px solid var(--line)', background: 'rgba(209,0,113,0.02)' }}>
                    <MapPin className="h-3.5 w-3.5" style={{ color: 'var(--magenta-bold)' }} />
                  ...
                  {daerahList.map(d => (
                    ...
                    >
                      <div>
                        <p className="m-0 text-sm font-medium" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-heading)' }}>
                          {d.kota}
                        </p>
