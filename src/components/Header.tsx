...
function DropdownMenu({ group, isOpen, onToggle, onClose }: DropdownProps) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {isOpen && (
        <div className="absolute left-0 top-full z-50 mt-2 w-72 overflow-hidden rounded-2xl border border-line bg-white shadow-[0_20px_60px_rgba(157,22,124,0.12),0_8px_24px_rgba(0,0,0,0.08)] rise-in">
          <div className="p-2">
            {group.items.map((item) => (
              ...
              >
                <span className="mt-0.5 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-[rgba(209,0,113,0.08)] text-[var(--color-magenta-bold)] transition-colors group-hover:bg-[rgba(209,0,113,0.14)]">
                  {item.icon}
                </span>
                <div>
                  <p className="m-0 text-sm font-semibold text-[var(--color-text-primary)] font-[var(--font-heading)]">
                    {item.label}
                  </p>
                  {item.desc && (
                    <p className="m-0 text-xs text-[var(--color-text-secondary)] mt-0.5">
                      {item.desc}
                    </p>
                  )}
                </div>
        ...
        <nav className="page-wrap flex items-center gap-4 py-3 lg:py-3.5">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              ...
              className="lg:hidden flex h-9 w-9 items-center justify-center rounded-xl border border-[var(--color-line)] bg-white text-[var(--color-text-secondary)] transition hover:border-[rgba(209,0,113,0.3)] hover:text-[var(--color-magenta-bold)]"
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-[var(--color-line)] px-5 py-4">
          <Link
            ...
            onClick={() => setMobileOpen(false)}
          >
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-[var(--color-magenta-bold)] to-[var(--color-magenta-deep)]">
          ...
          <button
            type="button"
            onClick={() => setMobileOpen(false)}
            className="flex h-8 w-8 items-center justify-center rounded-lg text-[var(--color-text-secondary)] hover:bg-[rgba(209,0,113,0.06)] hover:text-[var(--color-magenta-bold)]"
        ...
        <div className="flex-1 overflow-y-auto py-4">
          {NAV_GROUPS.map((group) => (
            <div key={group.label} className="mb-1">
              <button
                ...
                className="flex w-full items-center justify-between px-5 py-3 text-left font-[var(--font-heading)] font-semibold text-sm text-[var(--color-text-primary)] hover:bg-[rgba(209,0,113,0.04)]"
              >
                {group.label}
                <ChevronDown
                  className={`h-4 w-4 text-[var(--color-text-secondary)] transition-transform duration-200 ${mobileExpanded === group.label ? "rotate-180" : ""}`}
                />
              </button>
          ...
          <Link
            to="/kontak"
            onClick={() => setMobileOpen(false)}
            className="flex items-center gap-2 px-5 py-3 font-[var(--font-heading)] font-semibold text-sm text-[var(--color-text-primary)] hover:bg-[rgba(209,0,113,0.04)] no-underline"
          >
            <MessageCircle className="h-4 w-4 text-[var(--color-magenta-bold)]" />
            Kontak
          </Link>
        </div>

        {/* Drawer footer CTA */}
        <div className="border-t border-[var(--color-line)] p-5 space-y-3">
          <a
            href="https://play.google.com/store/apps/details?id=com.wafaindonesia"
            target="_blank"
            rel="noreferrer"
